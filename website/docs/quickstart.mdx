...
:::caution ¡No lo olvides!
  Recuerda reemplazar nuestra llave de ejemplo <code>sk_test_API_KEY</code>
  con tu propia llave secreta.
:::

Este ejemplo muestra los campos mínimos requeridos para crear una factura de tipo `Ingreso` en una sola llamada.

Para conocer a fondo todas las opciones disponibles al crear una factura, así como una descripción
detallada de cada campo, consulta la [referencia del método Crear Factura](/api/#operation/createInvoice).

:::info
  Por default se considera que el precio del producto tiene impuestos incluídos.
  Facturapi se encarga de desglosar los impuestos del producto (siendo IVA 16% el default)
...
:::success ¡Listo! 🎉
Para este punto tu factura fue creada exitosamente, y si estás en ambiente Live, enviada al SAT.
:::

¿Cómo asegurarme de que se envió al SAT? La propiedad `verification_url` del objeto Invoice de la respuesta
