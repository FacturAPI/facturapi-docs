...
**taxes[].ieps_mode**<br><small>opcional</small> | string | "sum_before_taxes" | <p>Sólo aplica si el tipo de impuesto es `"IEPS"`. Indica la manera de cobrar el impuesto, y puede tener los valores:</p><ul><li>`"sum_before_taxes"`: Aplica primero el IEPS al subtotal y usa el resultado como base del resto de impuestos en el producto.</li><li>`"break_down"`: Cobra y desgloza el IEPS al mismo nivel que el resto de los impuestos en el producto.</li><li>`unit`: Aplica el IEPS antes del precio unitario, y usa el precio unitario original como base para el resto de impuestos.</li></ul><p>Consulta con tu contador qué caso aplica para tu giro de empresa y producto.</p>
